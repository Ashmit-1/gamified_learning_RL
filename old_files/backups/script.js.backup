...
async function handleAnswer(selectedOption, button) {
    ...
    // Check if game already ended locally
    if (gameResult !== null || pendingLose) {
        // We wait for the timeout in zombie update to show result
        return;
    }
    ...
    try {
        ...
        setTimeout(() => {
            if (gameResult === null && !pendingLose) {
                ...
                // If backend runs out, we might need to end or repeat? 
                // For now, assume backend provides questions or generates if low.
                renderQuestion(currentQuestion);
            }
        }, 1200);
