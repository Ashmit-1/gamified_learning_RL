...
class StudentSession:
    ...
    def get_next_question(self) -> Dict:
        ...
        # Check if category is empty and try alternatives
        if not self.pool.get(category) or len(self.pool[category]) == 0:
            print(f"[SESSION WARNING] Category '{category}' is EMPTY!")
            # Try other categories in order of preference
            alternative_categories = ["easy_conceptual", "medium_application", "hard_problem_solving", "easy_revision"]
            found_alternative = False
            
            for alt_category in alternative_categories:
                if self.pool.get(alt_category) and len(self.pool[alt_category]) > 0:
                    print(f"[SESSION] Switching to alternative category '{alt_category}'")
                    category = alt_category
                    self.current_action = [k for k, v in self.id_to_category.items() if v == alt_category][0]
                    action_name = ACTION_NAMES[self.current_action]
                    found_alternative = True
                    break
            
            # If all categories are empty, raise error
            if not found_alternative:
                raise IndexError(f"All pool categories exhausted for topic '{self.topic}'")
    ...
    def process_answer(
        ...
        # Save checkpoint every 10 questions for safety
        if self.question_count % 10 == 0:
            self.agent.save_checkpoint(self.topic, self.session_id)
