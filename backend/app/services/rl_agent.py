...
class QLearningAgent:
    ...
    def save_policy(self, topic: str, save_dir: str = "q_tables"):
        ...
        try:
            print(f"[RL] Attempting to save Q-table for topic '{topic}' to dir '{save_dir}'")
            ...
            filepath = save_path / f"{topic.lower().replace(' ', '_')}.json"
            print(f"[RL] Saving to filepath: {filepath.absolute()}")
            
            # Verify the directory exists
            if not save_path.exists():
                print(f"[RL] Warning: Directory {save_path} does not exist!")
            ...
            if filepath.exists():
                print(f"[RL] Successfully saved Q-table for topic '{topic}' (ε={self.epsilon:.3f}, episodes={self.episodes})")
                print(f"[RL] File size: {filepath.stat().st_size} bytes")
            else:
                print(f"[RL] Warning: File {filepath} was not created!")
                
            return True
        except Exception as e:
            print(f"[RL] Error saving Q-table for topic '{topic}': {e}")
            import traceback
            traceback.print_exc()
            return False
    
    def save_checkpoint(self, topic: str, session_id: str, save_dir: str = "q_tables/checkpoints"):
        """Save current learning progress as checkpoint."""
        try:
            checkpoint_topic = f"{topic}_checkpoint_{session_id}"
            print(f"[RL] Saving checkpoint for topic '{topic}' with session '{session_id}'")
            result = self.save_policy(checkpoint_topic, save_dir)
            if result:
                print(f"[RL] Checkpoint saved successfully for {topic}")
            else:
                print(f"[RL] Checkpoint save failed for {topic}")
            return result
        except Exception as e:
            print(f"[RL] Checkpoint save failed for {topic}: {e}")
            import traceback
            traceback.print_exc()
            return False
    ...
    def load_policy(self, topic: str, save_dir: str = "q_tables") -> bool:
        ...
        """
        print(f"[RL] Attempting to load policy for topic '{topic}' from dir '{save_dir}'")
        filepath = Path(save_dir) / f"{topic.lower().replace(' ', '_')}.json"
        print(f"[RL] Looking for policy at: {filepath.absolute()}")
        
        # Also check checkpoint directory as fallback
        checkpoint_filepath = Path("q_tables/checkpoints") / f"{topic.lower().replace(' ', '_')}_checkpoint_*.json"
        print(f"[RL] Also checking checkpoint directory")
        
        if not filepath.exists():
            print(f"[RL] No saved policy found in main directory for topic '{topic}'")
            # Check if directory exists at all
            if not Path(save_dir).exists():
                print(f"[RL] Save directory '{save_dir}' does not exist")
            
            # Look for any checkpoint files
            checkpoint_files = list(Path("q_tables/checkpoints").glob(f"{topic.lower().replace(' ', '_')}_checkpoint_*.json"))
            if checkpoint_files:
                print(f"[RL] Found checkpoint files: {[f.name for f in checkpoint_files]}")
                # Use the most recent checkpoint
                checkpoint_filepath = max(checkpoint_files, key=lambda f: f.stat().st_mtime)
                print(f"[RL] Loading policy from most recent checkpoint: {checkpoint_filepath}")
        ...
        try:
            print(f"[RL] Loading policy from: {filepath}")
            ...
            print(f"[RL] Error loading policy for topic '{topic}': {e}")
            import traceback
            traceback.print_exc()
            return False
